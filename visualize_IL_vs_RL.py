...
imitation_data_path = os.path.abspath(
    os.path.expanduser(os.path.join("~/ModelBasedDDPG/imitation_data", scenario))
)
rl_trajectories_data_path = os.path.abspath(
    os.path.expanduser(
...
def process_poses(
    target_poses, x_coordinate_range=(0.0, 0.13), z_coordinate_range=(0.3, 0.45)
):
    return [
        p
        for p in target_poses
        if x_coordinate_range[0] <= p[0] <= x_coordinate_range[1]
        and z_coordinate_range[0] <= p[1] <= z_coordinate_range[1]
    ]
    ...
    while trajectories_seen < trajectory_limitation:
        ...
        for dirpath, dirnames, filenames in os.walk(global_step_dir):
            for filename in filenames:
                ...
                joints = trajectory[1]
                poses = [
                    openrave_manager.get_target_pose(tuple([0.0] + list(j)))
                    for j in joints
                ]
...
rl_transitions = process_rl_files(
    rl_trajectories_data_path, number_of_imitation_files * 1000
)
        ...
        for filename in filenames[:limit_files]:
            ...
            with bz2.BZ2File(source_file, "r") as compressed_file:
                ...
                for trajectory in all_trajectories:
                    poses = trajectory[1]
                    poses_after_filter = process_poses(
                        [p[target_point_tuple] for p in poses]
                    )
                    result.extend(poses_after_filter)
        return result


il_transitions = process_dir(
    os.path.join(imitation_data_path, "train"), number_of_imitation_files
)
