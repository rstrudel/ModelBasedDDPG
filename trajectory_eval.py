...
class TrajectoryEval:
    def __init__(self, config, rollout_manager, results_directory):
        ...
        potential_points_path = os.path.join(
            self.results_directory, "potential_points.p"
        )
        pickle.dump(
            PotentialPoint.from_config(config), open(potential_points_path, "w")
        )
        self._is_vision = config["model"]["consider_image"]

    def eval(
        self, global_step, number_of_episodes, is_train=False, return_episodes=False
    ):
        ...
        episode_results = self.rollout_manager.generate_episodes(
            number_of_episodes, is_train
        )
    ...
    def save_trajectory(
        self, trajectory, goal_pose, path_index, header, global_step, workspace_id
    ):
        # get the joints
