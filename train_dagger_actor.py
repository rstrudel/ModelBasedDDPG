...
train_transition_data_loader = TransitionDataLoader(
    train_transition_files, config["general"]["train_transition_files"]
)
...
class OpenravePlannerProcess(multiprocessing.Process):
    def __init__(
        self, config, generate_plan_queue, result_queue, planner_specific_queue
    ):
    ...
    def _run_query(self, query_params):
        ...
        traj = self.openrave_interface.openrave_manager.plan(
            start_state, goal_state, config["openrave_rl"]["planner_iterations"]
        )
        if traj is not None:
            split_traj = OpenraveTrajectoryGenerator.split_trajectory(
                traj, config["openrave_rl"]["action_step_size"]
            )
    ...
    def _run_main_loop(self):
        while True:
            try:
                ...
                pass
            try:
                next_actor_specific_task = self.planner_specific_queue.get(
...
class OpenravePlannerManager:
    def __init__(self, config):
        ...
        self.planner_specific_queues = [
            multiprocessing.JoinableQueue() for _ in range(actor_processes)
        ]

        self.planner_processes = [
            OpenravePlannerProcess(
                copy.deepcopy(config),
                self.episode_generation_queue,
                self.episode_results_queue,
...
with tf.Session(
    ...
    for epoch in range(epochs):
        ...
        eval_result = train_trajectory_eval.eval(
            current_global_step,
            config["model"]["dagger_episodes_per_epoch"],
            is_train=True,
            return_episodes=True,
        )
        train_transition_data_loader.add_extra_transitions(
            create_dagger_transitions(eval_result[-1])
        )
