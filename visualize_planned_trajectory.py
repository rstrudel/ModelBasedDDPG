...
trajectories_dir = os.path.abspath(os.path.expanduser("/home/tom/ModelBasedDDPG/"))
if is_imitation:
    trajectories_dir = os.path.join(
        trajectories_dir, "imitation", scenario, "trajectories"
    )
    if is_train:
...
def main():
    ...
    search_key = os.path.join(
        model_dir, global_step, "{}_{}.p".format(message, path_id)
    )
    ...
    if display_start_goal_end_spheres:
        ...
        start_sphere = create_sphere(
            "start", trajectory_spheres_radius, openrave_manager
        )
